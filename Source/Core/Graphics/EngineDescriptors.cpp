...
namespace TulparEngine {
    ...
    std::unique_ptr<EngineDescriptorSetLayout> EngineDescriptorSetLayout::Builder::build() const {
        return std::make_unique<EngineDescriptorSetLayout>(bindings);
    }
    
    EngineDescriptorSetLayout::EngineDescriptorSetLayout(std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> bindings) : bindings{bindings} {
    ...
    EngineDescriptorSetLayout::~EngineDescriptorSetLayout() {
        vkDestroyDescriptorSetLayout(EngineVulkanManager::GetInstance().getDevice(), descriptorSetLayout, nullptr);
    }
    ...
    std::unique_ptr<EngineDescriptorPool> EngineDescriptorPool::Builder::build() const {
        return std::make_unique<EngineDescriptorPool>(maxSets, poolFlags, poolSizes);
    }
    ...
    EngineDescriptorPool::EngineDescriptorPool(
        ...
        if (vkCreateDescriptorPool(EngineVulkanManager::GetInstance().getDevice(), &descriptorPoolInfo, nullptr, &descriptorPool) !=
            VK_SUCCESS) {
    ...
    EngineDescriptorPool::~EngineDescriptorPool() {
        vkDestroyDescriptorPool(EngineVulkanManager::GetInstance().getDevice(), descriptorPool, nullptr);
    }
    ...
    bool EngineDescriptorPool::allocateDescriptor(const VkDescriptorSetLayout descriptorSetLayout, VkDescriptorSet &descriptor) const {
        ...
        if (vkAllocateDescriptorSets(EngineVulkanManager::GetInstance().getDevice(), &allocInfo, &descriptor) != VK_SUCCESS) {
            return false;
        }
        return true;
    }
    ...
    void EngineDescriptorPool::resetPool() {
        vkResetDescriptorPool(EngineVulkanManager::GetInstance().getDevice(), descriptorPool, 0);
    }
    ...
    void EngineDescriptorWriter::overwrite(VkDescriptorSet &set) {
        ...
        vkUpdateDescriptorSets(EngineVulkanManager::GetInstance().getDevice(), writes.size(), writes.data(), 0, nullptr);
    }
}